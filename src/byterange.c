...
static int ap_set_byterange(request_rec *r, apr_off_t clength,
  ...
  *indexes = apr_array_make(r->pool, ranges, sizeof(indexes_t));
  ...
  ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,
                APLOGNO(01583) "Range: %s | %s (%d : %d : %" APR_OFF_T_FMT ")",
 ...
 */
static apr_status_t send_416(const dav_resource *resource, ap_filter_t *output,
 ...
 */
static dav_error *deliver_seek(const dav_resource *resource,
                               openedDataObjInp_t *data_obj, size_t pos) {
 ...
 */
static dav_error *deliver_file_bytes(const dav_resource *resource,
                                     openedDataObjInp_t *data_obj,
                                     ap_filter_t *output,
                                     apr_bucket_brigade *bb, size_t seek_pos,
                                     size_t bytes_to_read, size_t *total_read) {
  ...
  *total_read = 0;
  ...
  while (*total_read < bytes_to_read) {
    ...
    bytes_read =
        rcDataObjRead(resource->info->rods_conn, data_obj, &read_buffer);
    ...
    if (bytes_read < 0) {
      ...
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                    "rcDataObjRead failed: %d = %s", bytes_read,
                    get_rods_error_msg(bytes_read));
...
dav_error *davrods_byterange_deliver_file(const dav_resource *resource,
  ...
  core_dir_config *core_conf =
      ap_get_core_module_config(resource->info->r->per_dir_config);

  int max_ranges = ((core_conf->max_ranges >= 0 ||
                     core_conf->max_ranges == AP_MAXRANGES_UNLIMITED)
                        ? core_conf->max_ranges
                        : AP_DEFAULT_MAX_RANGES);
  int max_overlaps = ((core_conf->max_overlaps >= 0 ||
                       core_conf->max_overlaps == AP_MAXRANGES_UNLIMITED)
                          ? core_conf->max_overlaps
                          : AP_DEFAULT_MAX_OVERLAPS);
  int max_reversals = ((core_conf->max_reversals >= 0 ||
                        core_conf->max_reversals == AP_MAXRANGES_UNLIMITED)
  ...
  int overlaps = 0;
  int reversals = 0;
  int num_ranges = ap_set_byterange(resource->info->r, obj_length, &indexes,
                                    &overlaps, &reversals);

  if (num_ranges) {
    ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, resource->info->r,
                  "Ranges: %d, overlaps: %d, reversals: %d", num_ranges,
                  overlaps, reversals);
  }
  ...
  if (num_ranges == 0 || (max_ranges >= 0 && num_ranges > max_ranges) ||
    ...
    size_t total_read = 0;
    dav_error *err = deliver_file_bytes(resource, data_obj, output, bb, 0,
                                        obj_length, &total_read);
    return err;
  ...
  if (num_ranges > 1) {
    ...
    ap_set_content_type(
        r, apr_pstrcat(r->pool, "multipart", use_range_x(r) ? "/x-" : "/",
                       "byteranges; boundary=", ap_multipart_boundary, NULL));
    bound_head = apr_pstrcat(r->pool, CRLF "--", ap_multipart_boundary,
  ...
  for (int i = 0; i < indexes->nelts; i++, idx++) {
    ...
    if (num_ranges == 1) {
      apr_table_setn(r->headers_out, "Content-Range",
                     apr_psprintf(r->pool, "bytes " BYTERANGE_FMT, range_start,
                                  range_end, obj_length));
    } else {
      char *ts;
      ...
      ts = apr_psprintf(r->pool, BYTERANGE_FMT CRLF CRLF, range_start,
                        range_end, obj_length);
    ...
    size_t total_read = 0;
    dav_error *err =
        deliver_file_bytes(resource, data_obj, output, bb, range_start,
                           range_end - range_start + 1, &total_read);
    if (err)
      return err;
  }
  ...
  if (num_ranges > 1) {
    ...
    // Add the final boundary.
        ...
        apr_pstrcat(r->pool, CRLF "--", ap_multipart_boundary, "--" CRLF, NULL);
    ap_xlate_proto_to_ascii(end, strlen(end));
    apr_bucket *e =
        apr_bucket_pool_create(end, strlen(end), r->pool, c->bucket_alloc);
    APR_BRIGADE_INSERT_TAIL(bb, e);
  }
