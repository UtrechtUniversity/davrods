...
static dav_error *get_davrods_pool(request_rec *r, apr_pool_t **pool) {
  ...
  davrods_dir_conf_t *conf =
      ap_get_module_config(r->per_dir_config, &davrods_module);
  assert(conf);
  ...
  //
  // - Anonymous-mode connections may or may not have an existing pool and
  // rods_conn.
  ...
  *pool = NULL;
  int status =
      apr_pool_userdata_get((void **)pool, "davrods_pool", r->connection->pool);
  if (!status && *pool) {
    // We have a pool.
    ...
    if (!status && rods_conn) {
      ...
      if (DAVRODS_CONF(conf, anonymous_mode) == DAVRODS_ANONYMOUS_MODE_ON) {
        ...
        // existing connection.
        bool can_reuse = davrods_user_can_reuse_connection(
            r, DAVRODS_CONF(conf, anonymous_auth_username),
        ...
        bool can_reuse_maybe = davrods_user_can_reuse_connection(r, NULL, NULL);
        ...
        if (can_reuse_maybe) {
          ...
          status = apr_pool_userdata_get(
              &flag_val, "davrods_request_was_basic_authed", main_req->pool);
          ...
          if (status == 0 && flag_val) {
            ...
            // This request is not basic-authed, yet anonymous mode is turned
            // off. This is likely a configuration error.
          }
        }
      }
  ...
  if (DAVRODS_CONF(conf, anonymous_mode) == DAVRODS_ANONYMOUS_MODE_ON) {
    ...
    authn_status result =
        check_rods(r, DAVRODS_CONF(conf, anonymous_auth_username),
                   DAVRODS_CONF(conf, anonymous_auth_password), false);

    if (result == AUTH_GRANTED) {
      int status = apr_pool_userdata_get((void **)pool, "davrods_pool",
                                         r->connection->pool);
      assert(status == 0 &&
             *pool); // Logic error - pool must have been set by auth module.
      ...
      return dav_new_error(
          r->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
          "Anonymous mode is enabled but Davrods couldn't log in "
          "with the configured anonymous credentials (option "
          "DavrodsAnonymousLogin) "
    ...
    return dav_new_error(
        ...
        "Either enable anonymous access mode (DavrodsAnonymousMode On, Require "
        "all granted) "
        "or enable Davrods' basic auth provider (Require valid-user, AuthType "
        "Basic, AuthBasicProvider irods).");
  }
}
...
static dav_error *set_rods_path_from_uri(dav_resource *resource) {
  // Set iRODS path and relative URI properties of the resource context based on
  // the resource URI.
  ...
  if (resource->info->root_dir && strlen(resource->info->root_dir) > 1) {
    // We expect the URI to contain the specified root_dir (<- the Location in
    // which davrods runs).
  ...
  if (strlen(prefixed_path) >= MAX_NAME_LEN) {
    ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                  "Generated an iRODS path exceeding iRODS path length limits "
                  "for URI <%s>",
                  uri);
    return dav_new_error(resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
  ...
  ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, resource->info->r,
                "Mapped URI <%s> to rods path <%s>", uri,
                resource->info->rods_path);
...
/**
 * \brief Copy private resource info to a new resource, discarding
...
static const char *get_rods_root(apr_pool_t *davrods_pool, request_rec *r) {
  ...
  if (DAVRODS_CONF(conf, rods_exposed_root_type) == DAVRODS_ROOT_ZONE_DIR) {
    root = apr_pstrcat(davrods_pool, "/", DAVRODS_CONF(conf, rods_zone), NULL);
  } else if (DAVRODS_CONF(conf, rods_exposed_root_type) ==
             DAVRODS_ROOT_HOME_DIR) {
    root = apr_pstrcat(davrods_pool, "/", DAVRODS_CONF(conf, rods_zone),
                       "/home", NULL);
  } else if (DAVRODS_CONF(conf, rods_exposed_root_type) ==
             DAVRODS_ROOT_USER_DIR) {
    const char *username = NULL;
    int status =
        apr_pool_userdata_get((void **)&username, "username", davrods_pool);
...
static const char *get_session_ticket(const dav_resource *resource) {
  ...
  int status = apr_pool_userdata_get((void *)&active_ticket, "active_ticket",
                                     resource->info->davrods_pool);
  assert(status == 0);
 ...
 */
static void set_session_ticket(const dav_resource *resource,
                               const char *ticket) {
...
#define RETURN_ERROR_IF_TICKET_ACTIVE_AND_READONLY(resource)                   \
  if (DAVRODS_CONF(resource->info->conf, ticket_mode) ==                       \
    ...
    ap_log_rerror(                                                             \
        ...
        resource->uri);                                                        \
    return dav_new_error(                                                      \
        resource->pool, HTTP_FORBIDDEN, 0, 0,                                  \
        "Ticket write actions are disallowed by this server");                 \
  }

/**
 * \brief Query iRODS for information pertaining to a certain resource and fill
 * in those resource properties.
 */
static dav_error *get_dav_resource_rods_info(dav_resource *resource) {
  ...
  if (status < 0) {
    ...
    ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, r,
                  ...
                  : stat_out->objType == COLL_OBJ_T ? "collection"
                                                    : "thingy",
                  stat_out->objSize);
...
static dav_error *dav_repo_get_resource(request_rec *r, const char *root_dir,
  ...
  int status = apr_pool_userdata_get((void **)&res_private->rods_conn,
                                     "rods_conn", res_private->davrods_pool);
  assert(status == 0 && res_private->rods_conn);

  // Obtain iRODS environment.
  status = apr_pool_userdata_get((void **)&res_private->rods_env, "env",
                                 res_private->davrods_pool);
  ...
  if (DAVRODS_CONF(res_private->conf, ticket_mode) != DAVRODS_TICKET_MODE_OFF) {
    ...
    set_session_ticket(resource,
                       apr_table_get(r->subprocess_env, DAVRODS_TICKET_VAR));
  }
...
static dav_error *dav_repo_open_stream(const dav_resource *resource,
  ...
  if (mode == DAV_MODE_WRITE_SEEKABLE ||
      (mode == DAV_MODE_WRITE_TRUNC &&
       DAVRODS_CONF(resource->info->conf, tmpfile_rollback) ==
           DAVRODS_TMPFILE_ROLLBACK_OFF)) {
    ...
    // If the TmpfileRollback config option is set, we create a temporary file
    // when in truncate mode.

    // Think up a semi-random filename that's unlikely to exist in this
    // directory.
    int cheapsum = 0;
    ...
    // TODO: Statting the parent collection is overkill, create a separate
    // function for this.
    dav_resource *parent;
    ...
    //       always be the case.
    stream->write_path =
        apr_psprintf(stream->pool, "%s/.davrods-tx-%04x-%08lx",
  ...
  if (strlen(stream->write_path) >= MAX_NAME_LEN) {
    ...
    ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                  "Generated a temporary filename exceeding iRODS MAX_NAME_LEN "
                  "limits: <%s>. Aborting open_stream().",
                  stream->write_path);
    return dav_new_error(
  ...
  if (strcmp(stream->write_path, resource->info->rods_path) == 0 &&
    ...
    // We are overwriting an existing data object without the use of a temporary
    // file.
    ...
    if ((status = rcDataObjOpen(resource->info->rods_conn, open_params)) >= 0) {
      ...
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                    "rcDataObjOpen failed for <%s>: %d = %s",
                    open_params->objPath, status, get_rods_error_msg(status));
      return dav_new_error(resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
    ...
    if ((status = rcDataObjCreate(resource->info->rods_conn, open_params)) >=
      ...
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                    "rcDataObjCreate failed for <%s>: %d = %s",
                    open_params->objPath, status, get_rods_error_msg(status));
      return dav_new_error(resource->pool, httpCode, 0, 0,
  ...
  ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, resource->info->r,
                ...
                DAVRODS_CONF(resource->info->conf, rods_tx_buffer_size) / 1024);
...
static dav_error *stream_send_buffer(dav_stream *stream, const char *buffer,
  ...
  if (written < 0) {
    ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, stream->resource->info->r,
                  "rcDataObjWrite failed: %d = %s", written,
...
static dav_error *stream_ship_container(dav_stream *stream) {
  if (stream->container && stream->container_off) {
    dav_error *err =
        stream_send_buffer(stream, stream->container, stream->container_off);
...
static dav_error *dav_repo_write_stream(dav_stream *stream,
  ...
  if (!stream->container) {
    ...
    stream->container_size =
        DAVRODS_CONF(stream->resource->info->conf, rods_tx_buffer_size);
    stream->container_off = 0;
...
static dav_error *dav_repo_close_stream(dav_stream *stream, int commit) {
  ...
  if (commit) {
    if (strcmp(stream->write_path, resource->info->rods_path)) {
      ...
      WHISPER("Moving tempfile <%s> to destination <%s>\n", stream->write_path,
              resource->info->rods_path);
      ...
      if ((status = rcDataObjRename(resource->info->rods_conn,
        ...
        if (status == UNIX_FILE_RENAME_ERR) {
          ...
          return dav_new_error(
              ...
              " Probably caused by a uploading a file with the name of a "
              "former collection"
              " (fs directory was not removed when iRODS collection was "
              "removed)");
        } else {
          return dav_new_error(
              resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
    ...
    if (strcmp(stream->write_path, resource->info->rods_path)) {
      ...
      // We do not want to deal with the trash when removing partially uploaded
      // files with temporary filenames.
      addKeyVal(&unlink_params.condInput, FORCE_FLAG_KW, "");
      int status = rcDataObjUnlink(resource->info->rods_conn, &unlink_params);
      if (status < 0) {
        ap_log_rerror(APLOG_MARK, APLOG_WARNING, APR_SUCCESS, resource->info->r,
      ...
      if (resource->exists) {
        ...
        ap_log_rerror(
            ...
            "Cannot rollback write to object opened with WRITE_SEEKABLE - the "
            "original file may be trashed if writes were issued");
        ...
        // See above, we do not want to deal with the trash when removing
...
static dav_error *dav_repo_seek_stream(dav_stream *stream, apr_off_t abs_pos) {
  ...
  int status =
      rcDataObjLseek(stream->resource->info->rods_conn, &seek_inp, &seek_out);
  ...
  if (status < 0) {
    ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, stream->resource->info->r,
                  "rcDataObjLseek failed: %d = %s", status,
                  get_rods_error_msg(status));
    return dav_new_error(stream->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
...
static dav_error *dav_repo_set_headers(request_rec *r,
  ...
  if (resource->collection) {
    // A GET on a collection => client must be a web browser / standard HTTP
    // client. We will output an HTML directory listing.
...
static dav_error *deliver_file(const dav_resource *resource,
  ...
  if ((status = rcDataObjOpen(resource->info->rods_conn, &open_params)) < 0) {
    ...
    ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                  "rcDataObjOpen failed: %d = %s", status,
                  get_rods_error_msg(status));
  ...
  dav_error *err =
      davrods_byterange_deliver_file(resource, &data_obj, output, bb);
  if (err) {
    apr_brigade_destroy(bb);
    return err;
  }
...
static dav_error *walker(struct dav_repo_walker_private *ctx, int depth) {
  WHISPER(
      "Entered walker (%d/%s), depth is %d - Current object <%s> is a %s.\n",
      ctx->params->walk_type,
      ctx->params->walk_type == DAV_WALKTYPE_AUTH       ? "AUTH"
      : ctx->params->walk_type == DAV_WALKTYPE_NORMAL   ? "NORMAL"
      : ctx->params->walk_type == DAV_WALKTYPE_LOCKNULL ? "LOCKNULL"
      : ctx->params->walk_type & DAV_WALKTYPE_NORMAL    ? "NORMAL+"
                                                        : "?",
      depth, ctx->resource.info->rods_path,
      ctx->resource.collection ? "collection" : "data object");
  ...
  if (err) {
    WHISPER("Walker callback returned an error, aborting. description: %s",
            err->desc);
    return err;
  }
  ...
  int status =
      rclOpenCollection(ctx->resource.info->rods_conn,
  ...
  if (ctx->params->walk_type & DAV_WALKTYPE_LOCKNULL) {
    ...
    if (ctx->params->lockdb->hooks == davrods_dav_provider_locallock.locks) {
      ...
      dav_error *err = davrods_locklocal_get_locked_entries(db, &ctx->resource,
                                                            &locked_name);
      if (err)
        return err;
      ...
      for (; locked_name; locked_name = locked_name->next) {
        ...
        if (uri_len + 1 + strlen(name) >= MAX_NAME_LEN ||
          ...
          ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS,
                        ...
                        "Generated an uri or iRODS path exceeding iRODS path "
                        "length limits");
          return dav_new_error(ctx->resource.pool, HTTP_INTERNAL_SERVER_ERROR,
                               0, 0, "Path name too long");
        }
        if (strcmp(ctx->uri_buffer, "/") == 0) {
          strcat(ctx->uri_buffer, name);
        } else {
        ...
        if (err) {
          WHISPER("(LOCKNULL) Walker callback returned an error, aborting. "
                  "description: %s",
                  err->desc);
          return err;
        }
...
static dav_error *dav_repo_walk(const dav_walk_params *params, int depth,
  ...
  struct dav_resource_private *ctx_res_private =
      apr_pcalloc(params->root->pool, sizeof(*ctx_res_private));
  assert(ctx_res_private);

  copy_resource_context(ctx_res_private, ctx.params->root->info);

  // FIXME: Use pool provided by walker params.
  ctx_res_private->stat =
      apr_pcalloc(params->root->pool, sizeof(rodsObjStat_t));
  WHISPER("Private @ %p\n", ctx_res_private);
  ...
  if (strlen(ctx_res_private->r->uri) >= MAX_NAME_LEN) {
    ...
    return dav_new_error(ctx.params->root->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
                         0, "Request URI too long");
  }
...
static dav_error *dav_copy_walk_callback(dav_walk_resource *wres,
                                         int calltype) {
  ...
  WHISPER("COPY: At resource <%s> srcroot<%s>, dstroot<%s>\n", resource->uri,
          ctx->src_rods_root, ctx->dst_rods_root);
  ...
  if (src_len > src_root_len) {
    if (dst_root_len + (src_len - src_root_len) < MAX_NAME_LEN) {
      ...
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, resource->info->r,
                    "Generated a copy destination filename exceeding iRODS "
                    "MAX_NAME_LEN (%u) limits for source resource <%s> "
                    "(%lu+(%lu-%lu)). Aborting copy.",
                    MAX_NAME_LEN, resource->uri, dst_root_len, src_root_len,
                    src_len);
      return dav_new_error(
          resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
  ...
  if (resource->collection) {
    ...
    {
      const char *res =
          DAVRODS_CONF(resource->info->conf, rods_default_resource);
      if (res && strlen(res))
...
static dav_error *dav_repo_copy_resource(const dav_resource *src,
  ...
  dav_walk_params walk_params = {.walk_type = DAV_WALKTYPE_NORMAL,
...
static dav_error *dav_repo_move_resource(dav_resource *src, dav_resource *dst,
  ...
  if (status < 0) {
    ...
    if (status == UNIX_FILE_RENAME_ERR) {
      ...
      /*
      Jan  4 17:21:12 pid:15626 ERROR: [-]
      iRODS/server/api/src/rsFileRename.cpp:115:_rsFileRename :  status
      [UNIX_FILE_RENAME_ERR]  errno [Is a directory] -- message [fileRename
      failed for
      [/var/lib/irods/iRODS/Vault/home/chris/litmus/.davrods-tx-3a8e-568aaa08]
      to [/var/lib/irods/iRODS/Vault/home/chris/litmus/mvnoncoll]]
          [-]     iRODS/server/drivers/src/fileDriver.cpp:474:fileRename :
      status [UNIX_FILE_RENAME_ERR]  errno [Is a directory] -- message [failed
      to call 'rename']
                  [-]     libunixfilesystem.cpp:1106:unix_file_rename_plugin :
      status [UNIX_FILE_RENAME_ERR]  errno [Is a directory] -- message [Rename
      error for
      "/var/lib/irods/iRODS/Vault/home/chris/litmus/.davrods-tx-3a8e-568aaa08"
      to "/var/lib/irods/iRODS/Vault/home/chris/litmus/mvnoncoll", errno = "Is a
      directory", status = -528021.]
      */

      return dav_new_error(
          src->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
          "iRODS Unix FS resource error: UNIX_FILE_RENAME_ERR."
          " Probably caused by a renaming a file to the name of a former "
          "collection"
