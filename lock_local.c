        ...
        if (apr_err) {
            if (apr_err == APR_EOF)
                ...
                return dav_new_error(lockdb->info->pool, HTTP_INTERNAL_SERVER_ERROR,
                                     0, apr_err, "Could not iterate DBM keys.");
        }

        const char *locked_path = key.dptr + 1;

        if (
               strstr(locked_path, colpath) == locked_path
            ...
            davrods_locklocal_lock_list_t* llEntry = apr_palloc(
                lockdb->info->pool, sizeof(davrods_locklocal_lock_list_t)
            );
            assert(llEntry);
