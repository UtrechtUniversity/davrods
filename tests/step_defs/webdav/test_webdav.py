...
from conftest import configuration, roles
...
def webdav_url():
    ...
    return configuration.get("davrods_url", "https://data.davrods:8445")
...
def webdav_session(request):
    ...
    session.auth = (roles[user]["username"], roles[user]["password"])
...
def webdav_request_root(webdav_session):
    # list the root directory with a PROPFIND request.
...
@when(
    parsers.parse('a WebDAV "{method}" request for "{path}" is made with an invalid password'),
    target_fixture="webdav_response",
)
def webdav_request_invalid_password(user, method, path):
    # Use the real user name but an invalid password.
    ...
    return requests.request(
        method,
        url,
        auth=(roles[user]["username"], "invalid-password"),
        verify=False,
        timeout=60,
    )
