...
typedef struct {
  ...
  const char
      *rods_exposed_root; // Note: This is not necessarily a path, see below.
  ...
  enum {
    //                               rods_exposed_root conf value => actual path
    //                               used
    //                               ------------------------------------------------------------------------------
    DAVRODS_ROOT_CUSTOM_DIR = 1, //             <path>           => <path>
    DAVRODS_ROOT_ZONE_DIR,       //             Zone             => /<zone>
    DAVRODS_ROOT_HOME_DIR, //             Home             => /<zone>/home (not
                           //             the user's home collection!)
    DAVRODS_ROOT_USER_DIR, //             User             =>
                           //             /<zone>/home/<username>
