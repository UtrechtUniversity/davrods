...
#define dav_size_direct(a)                                                     \
  (1 + sizeof(dav_lock_discovery_fixed) + sizeof(apr_uuid_t) +                 \
   ((a)->owner ? strlen((a)->owner) : 0) +                                     \
   ((a)->auth_user ? strlen((a)->auth_user) : 0) + 2)

/* Stored indirect lock info - lock token and apr_datum_t */
#define dav_size_indirect(a)                                                   \
  (1 + sizeof(apr_uuid_t) + sizeof(time_t) + sizeof((a)->key.dsize) +          \
   (a)->key.dsize)
 ...
 */
static const char *
dav_generic_format_locktoken(apr_pool_t *p, const dav_locktoken *locktoken) {
...
static dav_error *dav_generic_really_open_lockdb(dav_lockdb *lockdb) {
  ...
  if (status) {
    err =
        dav_generic_dbm_new_error(lockdb->info->db, lockdb->info->pool, status);

    char *status_code_string =
        apr_psprintf(lockdb->info->pool, "%d", APR_TO_OS_ERROR(status));
    char *error_description = apr_pstrcat(
        lockdb->info->pool,
        "Could not open the lock database. Path = ", lockdb->info->lockdb_path,
        ", status = ", status_code_string, NULL);

    return dav_push_error(lockdb->info->pool, HTTP_INTERNAL_SERVER_ERROR,
                          DAV_ERR_LOCK_OPENDB, error_description, err);
  }
...
#if DAV_DEBUG
  if (lockdb->ro) {
    return dav_new_error(lockdb->info->pool, HTTP_INTERNAL_SERVER_ERROR, 0, 0,
  ...
  while (offset < val.dsize) {
    ...
    default:
      ...
      return dav_new_error(
          p, HTTP_INTERNAL_SERVER_ERROR, DAV_ERR_LOCK_CORRUPT_DB, 0,
          apr_psprintf(p,
                       "The lock database was found to "
                       "be corrupt. offset %" APR_SIZE_T_FMT ", c=%02x",
...
static dav_error *dav_generic_resolve(dav_lockdb *lockdb,
  ...
  if ((err = dav_generic_load_lock_record(
           lockdb, indirect->key, DAV_CREATE_LIST, &dir, &ind)) != NULL) {
  ...
  return dav_new_error(lockdb->info->pool, HTTP_INTERNAL_SERVER_ERROR,
...
static const char *dav_generic_get_supportedlock(const dav_resource *resource) {
  static const char supported[] = DEBUG_CR
      ...
      "<D:locktype><D:write/></D:locktype>" DEBUG_CR "</D:lockentry>" DEBUG_CR
      "<D:lockentry>" DEBUG_CR "<D:lockscope><D:shared/></D:lockscope>" DEBUG_CR
      "<D:locktype><D:write/></D:locktype>" DEBUG_CR "</D:lockentry>" DEBUG_CR;
...
static dav_error *
dav_generic_remove_locknull_state(dav_lockdb *lockdb,
                                  const dav_resource *resource) {
    ...
    if (!dav_compare_locktoken(locktoken, ip->locktoken)) {
      *lock = dav_generic_alloc_lock(lockdb, ip->key, locktoken);
      ...
      if (partial_ok) {
        ...
        if ((err = dav_generic_resolve(lockdb, ip, &dp, NULL, NULL)) != NULL) {
          /* ### push a higher-level desc? */
          return err;
        }
        (*lock)->scope = dp->f.scope;
...
static dav_error *dav_generic_remove_lock(dav_lockdb *lockdb,
  ...
  if (locktoken != NULL) {
    ...
    if ((err = dav_generic_load_lock_record(lockdb, key, DAV_CREATE_LIST, &dh,
                                            &ih)) != NULL) {
      /* ### maybe add a higher-level description */
      return err;
    }
  ...
  for (; key.dsize; apr_err = apr_dbm_nextkey(dbm, &key)) {
    if (apr_err) {
      if (apr_err == APR_EOF)
        ...
        return dav_new_error(lockdb->info->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
                             apr_err, "Could not iterate DBM keys.");
    }
    ...
    if (strstr(locked_path, colpath) == locked_path &&
      ...
      davrods_locklocal_lock_list_t *llEntry =
          apr_palloc(lockdb->info->pool, sizeof(davrods_locklocal_lock_list_t));
      assert(llEntry);
