...
static int set_exposed_root(davrods_dir_conf_t *conf,
                            const char *exposed_root) {
...
const davrods_dir_conf_t default_config = {
    ...
    .rods_exposed_root =
        "User", // NOTE: Keep this in sync with the option below.
    ...
    .html_emit_tickets =
        DAVRODS_HTML_EMIT_TICKETS_ON, // no effect unless tickets on.
...
#define MERGE(_prop)                                                           \
  conf->_prop = child->_prop    ? child->_prop                                 \
                : parent->_prop ? parent->_prop                                \
                                : conf->_prop
  ...
  const char *exposed_root = child->rods_exposed_root ? child->rods_exposed_root
...
static const char *cmd_davrodsserver(cmd_parms *cmd, void *config,
...
static const char *cmd_davrodsauthscheme(cmd_parms *cmd, void *config,
                                         const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
  if (!strcasecmp("Native", arg1)) {
    ...
    return "Invalid iRODS authentication scheme. Valid options are 'Native' or "
           "'PAM'.";
  }
  return NULL;
}

static const char *cmd_davrodsauthttl(cmd_parms *cmd, void *config,
                                      const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
  ...
  if (ttl <= 0) {
    ...
    return "Auth TTL is too high - please specify a value that fits in an "
           "int32_t (i.e.: no more than 2 billion hours).";
  } else {
...
static const char *cmd_davrodszone(cmd_parms *cmd, void *config,
                                   const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
  conf->rods_zone = arg1;
  return NULL;
...
static const char *cmd_davrodsdefaultresource(cmd_parms *cmd, void *config,
...
static const char *cmd_davrodsenvfile(cmd_parms *cmd, void *config,
                                      const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodsexposedroot(cmd_parms *cmd, void *config,
                                          const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
  if (set_exposed_root(conf, arg1) < 0) {
    if (conf->rods_exposed_root_type == DAVRODS_ROOT_CUSTOM_DIR &&
        (!strlen(conf->rods_exposed_root) ||
         conf->rods_exposed_root[0] != '/')) {
      return "iRODS exposed root must be one of 'Zone', 'Home', 'User' or a "
             "custom path starting with a '/'";
    } else {
...
static const char *cmd_davrodstxbufferkbs(cmd_parms *cmd, void *config,
                                          const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodsrxbufferkbs(cmd_parms *cmd, void *config,
                                          const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
  size_t kb = apr_atoi64(arg1);
...
static const char *cmd_davrodstmpfilerollback(cmd_parms *cmd, void *config,
                                              const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodslockdb(cmd_parms *cmd, void *config,
                                     const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodsanonymousmode(cmd_parms *cmd, void *config,
                                            const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodsanonymouslogin(cmd_parms *cmd, void *config,
                                             int argc, char *const *argv) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodstickets(cmd_parms *cmd, void *config,
                                      const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodshtmlemittickets(cmd_parms *cmd, void *config,
                                              const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodshtmlhead(cmd_parms *cmd, void *config,
                                       const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodshtmlheader(cmd_parms *cmd, void *config,
                                         const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodshtmlfooter(cmd_parms *cmd, void *config,
                                         const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
static const char *cmd_davrodsforcedownload(cmd_parms *cmd, void *config,
                                            const char *arg1) {
  davrods_dir_conf_t *conf = (davrods_dir_conf_t *)config;
...
const command_rec davrods_directives[] = {
    AP_INIT_TAKE2(DAVRODS_CONFIG_PREFIX "Server", cmd_davrodsserver, NULL,
                  ACCESS_CONF, "iRODS host and port to connect to"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "AuthScheme", cmd_davrodsauthscheme,
                  NULL, ACCESS_CONF,
                  "iRODS authentication scheme to use (either Native or PAM)"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "AuthTTLHours", cmd_davrodsauthttl,
                  NULL, ACCESS_CONF,
                  "Time-to-live of the temporary password in hours (only "
                  "applies to PAM currently)"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "Zone", cmd_davrodszone, NULL,
                  ACCESS_CONF, "iRODS zone"),
    AP_INIT_TAKE_ARGV(DAVRODS_CONFIG_PREFIX "DefaultResource",
                      cmd_davrodsdefaultresource, NULL, ACCESS_CONF,
                      "iRODS default resource (optional)"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "EnvFile", cmd_davrodsenvfile, NULL,
                  ACCESS_CONF,
                  "iRODS environment file (defaults to "
                  "/etc/httpd/irods/irods_environment.json)"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "ExposedRoot", cmd_davrodsexposedroot,
                  NULL, ACCESS_CONF,
                  "Root iRODS collection to expose (one of: Zone, Home, User, "
                  "or a custom path)"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "TxBufferKbs", cmd_davrodstxbufferkbs,
                  NULL, ACCESS_CONF,
                  "Amount of file KiBs to upload to iRODS at a time on PUTs"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "RxBufferKbs", cmd_davrodsrxbufferkbs, NULL,
        ACCESS_CONF,
        "Amount of file KiBs to download from iRODS at a time on GETs"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "TmpfileRollback",
                  cmd_davrodstmpfilerollback, NULL, ACCESS_CONF,
                  "Support PUT rollback through the use of temporary files on "
                  "the target iRODS resource"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "LockDB", cmd_davrodslockdb, NULL, ACCESS_CONF,
        "Lock database location, used by the davrods-locallock DAV provider"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "AnonymousMode",
                  cmd_davrodsanonymousmode, NULL, ACCESS_CONF,
                  "Anonymous mode On/Off switch"),
    AP_INIT_TAKE_ARGV(DAVRODS_CONFIG_PREFIX "AnonymousLogin",
                      cmd_davrodsanonymouslogin, NULL, ACCESS_CONF,
                      "Anonymous mode username and optional password"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "HtmlHead", cmd_davrodshtmlhead, NULL,
        ACCESS_CONF,
        "File that's inserted into HTML directory listings, in the head tag"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "HtmlHeader", cmd_davrodshtmlheader, NULL,
        ACCESS_CONF,
        "File that's inserted into HTML directory listings, in the body tag"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "HtmlFooter", cmd_davrodshtmlfooter, NULL,
        ACCESS_CONF,
        "File that's inserted into HTML directory listings, in the body tag"),
    AP_INIT_TAKE1(
        DAVRODS_CONFIG_PREFIX "Tickets", cmd_davrodstickets, NULL, ACCESS_CONF,
        "When set to ReadOnly, allows for submitting tickets to iRODS to gain "
        "read-only access to collections and data objects"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "HtmlEmitTickets",
                  cmd_davrodshtmlemittickets, NULL, ACCESS_CONF,
                  "When On, emits tickets received from the client in HTML "
                  "listing anchor query strings"),
    AP_INIT_TAKE1(DAVRODS_CONFIG_PREFIX "ForceDownload",
                  cmd_davrodsforcedownload, NULL, ACCESS_CONF,
                  "When On, prevents inline display of files in web browsers"),
